...
function parseWat (filename, buffer, options) {
  ...
  try {
    var parseResult_addr = Module._wabt_parse_wat(
      lexer.addr,
      features.addr,
      errors.addr
    )
...
function readWasm (buffer, options) {
  ...
  try {
    ...
    var result = Module._wabt_read_binary_result_get_result(
    ...
    const module_addr = Module._wabt_read_binary_result_release_module(
      readBinaryResult_addr
    )
    var result = new WasmModule(module_addr, errors)
    // Clear errors so it isn't destroyed below.
...
WasmModule.prototype.toText = function (options) {
  ...
  const result = Module._wabt_write_module_result_get_result(
...
WasmModule.prototype.toBinary = function (options) {
  ...
  const write_debug_names = booleanOrDefault(options.write_debug_names, false)
