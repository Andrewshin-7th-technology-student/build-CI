...
(function (global, factory) {
  ...
  const ie_version =
    ie &&
    (ie_upto10 ? document.documentMode || 6 : Number((edge || ie_11up)[1]))
  ...
  function isEmpty (obj) {
    for (const n in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, n) && obj[n]) {
        return false
      }
    }
    return true
  ...
  function signalCursorActivity (cm) {
    const arr = cm._handlers?.cursorActivity
    if (!arr) {
      return
    }
    const set =
  ...
  function resolveMode (spec) {
    if (
      ...
      Object.prototype.hasOwnProperty.call(mimeModes, spec.name)
    ) {
  ...
  function getMode (options, spec) {
    ...
    if (Object.prototype.hasOwnProperty.call(modeExtensions, spec.name)) {
      ...
      for (const prop in exts) {
        ...
        if (Object.prototype.hasOwnProperty.call(modeObj, prop)) {
  ...
  function getLineStyles (cm, line, updateFrontier) {
    if (!line.styles || line.styles[0] != cm.state.modeGen) {
      const state = getStateBefore(cm, lineNo(line))
  ...
  function getStateBefore (cm, n, precise) {
    ...
    let pos = findStartLine(cm, n, precise)
    let state = pos > doc.first && getLine(doc, pos - 1).stateAfter
    if (!state) {
  ...
  function interpretTokenStyle (style, options) {
    ...
    const cache = options.addModeClass
  ...
  function buildToken (builder, text, style, startStyle, endStyle, title, css) {
    ...
    const displayText = builder.splitSpaces
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (markers) {
        for (let k = 0; k < cm.options.gutters.length; ++k) {
          ...
          const found =
            Object.prototype.hasOwnProperty.call(markers, id) && markers[id]
          if (found) {
  ...
  function coordsChar (cm, x, y) {
    ...
    for (;;) {
      ...
      const mergedPos = merged?.find(0, true)
      if (
        merged &&
  ...
  function drawSelectionCursor (cm, head, output) {
    ...
    const cursor = output.appendChild(
  ...
  function maybeUpdateLineNumberWidth (cm) {
    ...
    if (last.length != display.lineNumChars) {
      const test = display.measure.appendChild(
        elt(
          'div',
          [elt('div', last)],
          'CodeMirror-linenumber CodeMirror-gutter-elt'
  ...
  function highlightWorker (cm) {
    ...
    const end = Number(new Date()) + cm.options.workTime
    ...
    doc.iter(
      ...
      function (line) {
        if (doc.frontier >= cm.display.viewFrom) {
          ...
          const tooLong = line.text.length > cm.options.maxHighlightLength
  ...
  function skipAtomicInSelection (doc, sel, bias, mayClear) {
    ...
    for (let i = 0; i < sel.ranges.length; i++) {
      ...
      const newHead = skipAtomic(doc, range.head, old?.head, bias, mayClear)
  ...
  function makeChangeFromHistory (doc, type, allowSelectionOnly) {
    if (doc.cm?.state.suppressEdits && !allowSelectionOnly) {
      return
    }
  ...
  const LeafChunk = function (lines) {
    const this$1 = this
  ...
  const BranchChunk = function (children) {
    const this$1 = this
  ...
  function onDragStart (cm, e) {
    if (ie && (!cm.state.draggingText || Number(new Date()) - lastDrop < 100)) {
      e_stop(e)
      return
    }
    if (signalDOMEvent(cm, e) || eventInWidget(cm.display, e)) {
  ...
  function normalizeKeyMap (keymap) {
    ...
    for (const keyname in keymap) {
      if (Object.prototype.hasOwnProperty.call(keymap, keyname)) {
  ...
  function keyName (event, noShift) {
    ...
    const base = keyNames[event.keyCode]
    let name = base
    ...
    delWrappedLineRight: function (cm) {
      return deleteNearSelection(cm, function (range) {
        ...
        const rightPos = cm.coordsChar(
          { left: cm.display.lineDiv.offsetWidth + 100, top },
          'div'
        )
  ...
  function onMouseDown (e) {
    ...
    switch (e_button(e)) {
      ...
      case 2:
        if (webkit) {
          cm.state.lastMiddleDown = Number(new Date())
        }
        if (start) {
  ...
  function leftButtonSelect (cm, e, start, type, addNew) {
    ...
    function extendTo (pos) {
      ...
      if (type == 'rect') {
        const ranges = []
        const tabSize = cm.options.tabSize
        const startCol = countColumn(
          getLine(doc, start.line).text,
          start.ch,
          tabSize
        )
        const posCol = countColumn(
  ...
  function CodeMirror (place, options) {
    ...
    for (const opt in optionHandlers) {
      if (Object.prototype.hasOwnProperty.call(optionHandlers, opt)) {
  ...
  function addEditorMethods (CodeMirror) {
    ...
    CodeMirror.prototype = {
      ...
      getHelpers: function (pos, type) {
        ...
        const found = []
        if (!Object.prototype.hasOwnProperty.call(helpers, type)) {
          return found
        }
      ...
      scrollIntoView: methodOp(function (range, margin) {
        ...
        if (range.from.line != null) {
          ...
          const sPos = calculateScrollPos(this, {
  ...
  function findPosH (doc, pos, dir, unit, visually) {
    ...
    if (unit == 'char') {
      ...
      const helper = doc.cm?.getHelper(pos, 'wordChars')
      for (let first = true; ; first = false) {
      ...
      if (node.nodeType == 1) {
        ...
        if (markerID) {
          const found = cm.findMarks(
            Pos(fromLine, 0),
            Pos(toLine + 1, 0),
            recognizeMarker(Number(markerID))
          )
  ...
  for (const prop in Doc.prototype) {
    if (
      Object.prototype.hasOwnProperty.call(Doc.prototype, prop) &&
