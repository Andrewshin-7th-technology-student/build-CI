ActiveRecord::Schema[7.0].define(version: 20_221_228_065_556) do
  ...
  create_table 'doc_assignments', id: :serial, force: :cascade do |t|
    ...
    t.index %w[repo_subscription_id doc_method_id], name: 'index_doc_assignments_on_repo_subscription_id_and_doc_method_id'
  end
  ...
  create_table 'doc_classes', id: :serial, force: :cascade do |t|
    ...
    t.string 'path'
    t.string 'file'
    t.index %w[repo_id doc_comments_count], name: 'index_doc_classes_on_repo_id_and_doc_comments_count'
  end
  ...
  create_table 'doc_methods', id: :serial, force: :cascade do |t|
    ...
    t.text 'comment'
    t.index %w[repo_id doc_comments_count], name: 'index_doc_methods_on_repo_id_and_doc_comments_count'
    t.index %w[repo_id id], name: 'index_doc_methods_on_repo_id_and_id'
    t.index %w[repo_id name path], name: 'index_doc_methods_on_repo_id_and_name_and_path', unique: true
  end
  ...
  create_table 'issue_assignments', id: :serial, force: :cascade do |t|
    ...
    t.index %w[repo_subscription_id delivered], name: 'index_issue_assignments_on_repo_subscription_id_and_delivered'
  end
  ...
  create_table 'issues', id: :serial, force: :cascade do |t|
    ...
    t.index %w[number repo_id], name: 'index_issues_on_number_and_repo_id', unique: true
    t.index %w[repo_id id], name: 'index_issues_on_repo_id_and_id', where: "((state)::text = 'open'::text)"
    t.index %w[repo_id number], name: 'index_issues_on_repo_id_and_number'
    t.index %w[repo_id state], name: 'index_issues_on_repo_id_and_state'
  ...
  create_table 'repo_labels', force: :cascade do |t|
    ...
    t.index %w[repo_id label_id], name: 'index_repo_labels_on_repo_id_and_label_id', unique: true
  ...
  create_table 'repo_subscriptions', id: :serial, force: :cascade do |t|
    ...
    t.index %w[repo_id user_id], name: 'index_repo_subscriptions_on_repo_id_and_user_id'
    t.index ['repo_id'], name: 'index_repo_subscriptions_on_repo_id'
    t.index %w[user_id last_sent_at], name: 'index_repo_subscriptions_on_user_id_and_last_sent_at'
  ...
  create_table 'repos', id: :serial, force: :cascade do |t|
    ...
    t.index %w[name user_name], name: 'index_repos_on_name_and_user_name', unique: true
  ...
  create_table 'users', id: :serial, force: :cascade do |t|
    ...
    t.index %w[private id created_at], name: 'index_users_on_private_and_id_and_created_at'
  ...
  CodeTriage::Application.routes.draw do
    ...
    resources :university, only: %i[show index]
    ...
    resources :repo_subscriptions, only: %i[create destroy update]
    ...
    scope format: false do
      resources :repos, only: %i[index new create] do
        collection do
          get :list
        end
      end
